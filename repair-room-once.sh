...
curl -fsSL --max-time 30 "https://raw.githubusercontent.com/endtree-FDE/goai-crisis-cloud-bridge/$REV/repair-room-once.mjs" -o "$WORK/repair-room-once.mjs"
printf '%s  %s\n' 'd64d6a53e2805477e28ed2104c26b58f714b8edf8078a6fc879950002b8c3a48' "$WORK/repair-room-once.mjs" | sha256sum -c -
REMOTE="/tmp/$(basename "$WORK")"
docker cp "$WORK" "$C:/tmp/" >/dev/null

echo '=== 4/5 OFFICIAL PREFLIGHT: ROOMS, JOINED MEMBERS, P5/P6 STATES ==='
docker exec "$C" node "$REMOTE/repair-room-once.mjs" --plan "$REMOTE/p5-snapshot.json" /tmp/juchang-roomfix-p5-p6-20260903
...
if docker exec "$C" node "$REMOTE/repair-room-once.mjs" --apply "$REMOTE/p5-snapshot.json" /tmp/juchang-roomfix-p5-p6-20260903; then
